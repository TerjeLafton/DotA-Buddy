...
class BotCog(commands.Cog):
    ...
    async def time(self, ctx):
        self.message = await ctx.send(f'```Match has not started yet!\n\n'
    ...
    async def clock(self):
        if self.message:
            if self.clock_running:
                if self.seconds == 5:
                    self.last_event = f'{self.minutes}:{self.seconds}: Bounty rune in 15 seconds!'
            ...
            await self.message.edit(content=f'```Match has started!\n\n'
